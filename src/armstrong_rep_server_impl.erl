-module(armstrong_rep_server_impl).
...
init([Module, Args, SocketArgs, Endpoint, Options]) ->
    ...
    lager:debug("Initializing rep server:
                 module: ~p, args: ~p,
                 endpoint: ~p,
                 input options: ~p,
                 final options: ~p",
    ...
    {ok, SupPid} = armstrong_rep_socket_sup:start_link(
...
terminate(Reason, State) ->
    lager:debug("Terminating rep server: reason: ~p, state: ~p",
                [Reason, State]),
